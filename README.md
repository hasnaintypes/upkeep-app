...
Only needed for `POST /api/projects/register` (programmatic project registration, authenticated
by a per-user API key generated from `/dashboard/api-keys` — see `src/features/api-keys/`):
