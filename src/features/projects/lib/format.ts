 ...
 * (e.g. "GET https://...", "TCP db.example.com:5432", or "DNS
 * example.com"). For `check_type = "tcp"`/`"dns"` this is always the
 * literal "TCP"/"DNS", not `method` -- the `method` column still holds its
 * unused `'GET'` default on a non-http-type project (see the
