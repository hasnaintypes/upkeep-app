 ...
 * one-at-a-time, so a shadcn `Select` fits better here than a set of
 * toggle-able `<Link>` chips would.
 *
 * Radix `Select` has no native `<select>`/form-GET semantics to build a
 * plain `<Link>` href per option, so this is a small Client Component that
 * navigates via `router.push` on change -- immediate (not debounced) since
 * selecting an option is already a complete, deliberate action, unlike a
 * free-text search box's keystroke-by-keystroke typing.
 *
