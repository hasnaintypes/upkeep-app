...
import { FEATURE_CARDS, FEATURE_CARDS_CONTENT } from "../constants/feature-cards";
import type { FeatureCardData } from "../types";

function CardIllustration({ illustration }: { illustration: FeatureCardData["illustration"] }) {
  if (illustration === "grid") {
    return (
  ...
  const src = illustration === "speed-lines" ? "/images/speed-lines.png" : "/images/precision-workflow.png";
  const alt = illustration === "speed-lines" ? "Speed lines illustration" : "Precision workflow illustration";

  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={src} alt={alt} className="w-full h-full object-cover" style={{ filter: "invert(1)" }} />
    </div>
  );
}
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-8 mb-16">
            ...
            >
              <p className="text-zinc-400 leading-relaxed">
                {FEATURE_CARDS_CONTENT.descriptionPrefix}{" "}
                <a href="#" className="text-white inline-flex items-center gap-1 hover:underline">
                  {FEATURE_CARDS_CONTENT.descriptionLinkText} <ChevronRight className="w-4 h-4" />
                </a>
              </p>
            </motion.div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {FEATURE_CARDS.map((card, index) => (
              ...
              >
                ...
                >
                  <CardIllustration illustration={card.illustration} />
                </div>
                <div
